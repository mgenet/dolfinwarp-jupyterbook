...
class TawhaiElasticMaterial(ElasticMaterial):
    ...
    def get_free_energy(self,
        ...
        ## J1
        # expo    = dolfin.exp(self.alpha * I1 * JF**(-2./3.))
        # Psi     = self.gamma * expo
        #
        # dpsidI1 = self.gamma * self.alpha * JF**(-2./3.) * expo
        # dpsidJ  = self.gamma * (self.alpha * I1 * (-2./3.) * JF**(-5./3.))
        # Sigma   = 2 * (dpsidI1 * I + dpsidJ * JF / 2 * C_inv)

        ## I1
        # expo    = dolfin.exp(self.alpha * I1)
        # Psi     = self.gamma * expo
        #
        # dpsidI1 = self.gamma * self.alpha * expo
        # Sigma   = 2 * (dpsidI1 * I)

        ## I1 & I2
        expo    = dolfin.exp(self.alpha * I1 + self.beta * I2)
        Psi     = self.gamma * expo

        dpsidI1 = self.gamma * self.alpha * expo
        dpsidI2 = self.gamma * self.beta * expo
        Sigma   = 2 * (dpsidI1 * I + dpsidI2 * (I1 * I - C))
