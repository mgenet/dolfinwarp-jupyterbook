...
class MicroPoroHyperelasticityProblem(HyperelasticityProblem):
    ...
    def __init__(self,
            ...
            mesh=None,
            mesh_bbox=None,
            vertices=None, # MG20230210: So basically mesh_bbox is not needed anymore?
            domains_mf=None,
            boundaries_mf=None,
            points_mf=None,
        ...
        if (mesh is not None):
            ...
            self.X_0 = dolfin.Constant([0.]*self.dim)
    ...
    def add_macroscopic_stress_component_qois(self,i,j):
        ...
        self.add_qoi(
            ...
            # expr=(material.sigma[i,j] * self.kinematics.J - (v - vs)/Vs0 * tv_P.val * dolfin.Identity(2)[i,j])/v * self.dV)
            expr=(material.sigma[i,j] * self.kinematics.J - (v/Vs0 - self.kinematics.J) * tv_P.val * dolfin.Identity(2)[i,j])/v * self.dV)
